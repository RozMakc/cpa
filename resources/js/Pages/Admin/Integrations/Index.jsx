...
export default function Index({integrations}) {
  ...
  const updateApiKey = async (integration_id) => {
      if (!confirm('Вы уверены, что хотите обновить ключ?')) {
          return;
      }
      setProcessing(integration_id);
      try {
        await router.post(route('integration.newkey', integration_id), {}, {
            preserveScroll: true
        });
  ...
  const deleteUser = (integration_id) => {
      if (!confirm('Вы уверены, что хотите удалить Интеграцию?')) {
          return;
      }

      setProcessing(integration_id);
      router.delete(route('integration.destroy', integration_id), {
  ...
  return (
    ...
    <div className="rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div class="flex flex-col justify-between gap-5 border-b border-gray-200 px-5 py-4 sm:flex-row sm:items-center dark:border-gray-800">
            <div>
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Интеграции</h3>
            </div>
            <div class="flex gap-3">
                <Link 
                    ...
                    href={route('integration.create')} data-discover="true">
                        Добавить Интеграцию
                </Link>
            </div>
        </div>
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            ...
            integrations.map((integration) => (
              <TableRow key={integration.id}>
                ...
                <TableCell className="px-5 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                  {app.url}/api/postback/{integration.name}
                </TableCell>

                <TableCell className="px-5 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                <div className="flex gap-2">
                  <Link href={route('integration.edit', integration.id)}>
                  ...
                  <div>
                  <PrimaryButton
                      onClick={() => updateApiKey(integration.id)}
                  ...
                  <div>
                  <PrimaryButton
                      onClick={() => deleteUser(integration.id)}
                      disabled={processing === integration.id}
                      className='w-fit !py-2 !px-3 bg-red-500 hover:bg-red-600'
                  >
                      {processing === integration.id ? (
